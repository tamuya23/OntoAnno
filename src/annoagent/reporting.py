...
<head>
  <meta charset="utf-8">
  <title>{{ project_name }} - AnnoAgent Report</title>
  <style>
...
<body>
  <h1>{{ project_name }} - AnnoAgent Report</h1>
  <div class="meta">
  ...
  <p class="muted">Generated by AnnoAgent.</p>
</body>
</html>
"""
...
def _prepare_report_figures(
    ...
    spec = {
        ...
        "parent": {
            "seurat_rds": outputs.get("annotate_parent", {}).get("parent_seurat_rds"),
            "cluster_col": outputs.get("annotate_parent", {}).get("cluster_col"),
            "label_col": "celltype_parent",
        },
        "reviewed_parent": {
            "seurat_rds": outputs.get("reviewed_parent", {}).get("seurat_rds"),
            "label_col": outputs.get("reviewed_parent", {}).get("label_col", "celltype_parent_reviewed"),
        },
        "subcluster": {
...
def _build_subcluster_payload(config: dict[str, Any], outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    subcluster_dir = Path(str(subcluster_outputs.get("subcluster_folder") or ""))
    parent_names = []
    if subcluster_dir.exists():
        parent_names = sorted(item.name.replace("_", " ") for item in subcluster_dir.iterdir() if item.is_dir())
...
def _resolve_subcluster_outputs(config: dict[str, Any], outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    subcluster_folder = subcluster_dir / "subclusters_res0.1"
    ...
    if subcluster_folder.exists():
        resolved.setdefault("subcluster_folder", str(subcluster_folder))
...
def _build_report_context(config: dict[str, Any], state: dict[str, Any], manifest: dict[str, Any], report_path: Path) -> dict[str, Any]:
    ...
    annotation = {
        "best_parent_resolution": outputs.get("annotate_parent", {}).get("best_resolution"),
        "cluster_col": outputs.get("annotate_parent", {}).get("cluster_col"),
...
def _render_pdf_report(context: dict[str, Any], report_path: Path) -> Path:
    ...
    doc.title(f"{context['project_name']} - AnnoAgent Report")
